...
def chinese_save_to_txt():
    ...
    o33_train = Openslr33(True)
    for i in range(len(o33_train)):
        wav_path, text, dur_start, dur_end = o33_train.get_naive_item(i)
        ccc.add(text)
    
    o33_test = Openslr33(False)
    for i in range(len(o33_test)):
        wav_path, text, dur_start, dur_end = o33_test.get_naive_item(i)
        ccc.add(text)
