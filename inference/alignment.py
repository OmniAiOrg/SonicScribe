...
@torch.no_grad()
def detect_language(
    model: "WhisperOfficial", mel: Tensor, tokenizer: WhisperTokenizer = None
...
class PyTorchInference(Inference):
    def __init__(self, model: "WhisperOfficial", initial_token_length: int):
        self.model: "WhisperOfficial" = model
...
class AlighnmentRules(LogitFilter):
    def __init__(self, prompt:str, tokenizer: WhisperTokenizer, sample_begin: int, template=['time1','order','hanzi1','hanzi2','note','time2']):
        prompt = prompt.replace("AP","<|AP|><|AP|>").replace("SP","<|SP|><|SP|>").replace("SL", "<|SL|><|SL|>")
        self.prompt = tokenizer.encode(prompt, allowed_special="all")
        print("alignment prompt",prompt,"to tokens",self.prompt)
    ...
    def apply(self, logits: Tensor, tokens: Tensor):
        for k in range(tokens.shape[0]):
            ...
            count = len(sampled_tokens)%len(self.template)
            current = self.template[count]
            if current in ['hanzi1', 'hanzi2']:
                i = 0 if current == 'hanzi1' else 1
                index = len(sampled_tokens)//len(self.template)*2 + i
                if index >= len(self.prompt):
                    return
                index = self.prompt[index]
                logits[k, :index] = -np.inf
                logits[k, index+1:] = -np.inf
...
class OpenCpopRules(LogitFilter):
    ...
    def apply(self, logits: Tensor, tokens: Tensor) -> None:
        for k in range(tokens.shape[0]):
            ...
            if current in ['time1', 'time2']:
                eot_prob = logits[k, self.tokenizer.eot].clone()
            ...
            if current == 'note':
                ...
                if tokens[k, -1] in [self.tokenizer.AP, self.tokenizer.SP, self.tokenizer.SL]:
                    ...
                    continue
                # hanzi can not cause rest
                logits[k, :self.tokenizer.notes_begin] = -np.inf
                logits[k, self.tokenizer.notes_end:] = -np.inf
        return logits
...
class DecodingTask:
    ...
    def __init__(self, model: "WhisperOfficial", options: DecodingOptions):
        self.model = model
        ...
        self.sot_sequence: Tuple[int] = tokenizer.singing_sequence
        ...
        self.logit_filters.append(AlighnmentRules(prompt=self.options.prompt, tokenizer=tokenizer, sample_begin=self.sample_begin))
    ...
    def _get_initial_tokens(self) -> Tuple[int]:
        ...
        if prompt := self.options.prompt:
            prompt_tokens = (
                self.tokenizer.encode(" " + prompt.strip(),disallowed_special=())
    ...
    def _main_loop(self, audio_features: Tensor, tokens: Tensor):
        ...
        try:
            for i in range(self.sample_len):
                ...
                # print('XXX3', self.decode_tokens(tokens))
    ...
    def decode_tokens(self, tokens:Tensor):
        return self.tokenizer.decode(tokens[0].tolist(), include_timestamp=True)
...
if __name__ == "__main__":
    ...
    import torchaudio
    device='cpu'
    oc_test = OpenCpop(train=False, key_filter=['audio', 'hanzi', 'note', 'start', 'end', 'waveform'])
    ...
    options = DecodingOptions(
        ...
        temperature=0.1,
        # prompt='雨淋湿了天空AP灰得更讲究SPAP',
        prompt='我和你吻别在无人的街让风痴笑我不能拒绝我和你吻别在狂乱的夜我的心等着迎接伤悲',
        # prefix='<|singing|>'
    )
    audio_file_path = 'assets/test_wav/output/wenbie/vocals.wav'
    mel = log_mel_spectrogram(audio_file_path, padding=N_SAMPLES)
    print("mel.shape", mel.shape)
    decode_result = decode(model, mel[:,:N_FRAMES], options)
    print(decode_result)
    diffsinger = opencpop_to_diffsinger(decode_result.tokens, model.tokenizer)
    print(diffsinger)
    ...
    for b in loader:
        ...
        torchaudio.save('logs/test_decoding.wav', b.waveform[0], 16000)
